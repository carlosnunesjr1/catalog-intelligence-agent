#!/usr/bin/env python3
"""Aponta key Nous para o proxy público (via Caddy) e captura request."""
import os, time, json
...
JS = """async () => {
  ...
  try {
    ...
    const apiKeyJson = JSON.stringify({ baseUrl: 'https://proxy-intercept.173-249-43-230.sslip.io/v1', apiKey: key });
...
with Camoufox(headless=False, humanize=True, os=("linux",)) as browser:
    ...
    time.sleep(3)
    try:
        print("[pp]", page.evaluate(JS))
    except Exception as e:
        print("[pp] eval fail:", e)
    ctx.close()
print("[pp] fim")
